...
def interpolate_3d_pos_encoding(pos_embed, D, H, W):
    ...
    N = pos_embed.shape[1]
    if num_patches == N and D == H and H == W:
        return pos_embed
    
    patch_pos_embed = pos_embed
    dim = pos_embed.shape[-1]

    # Reshape and permute for 3D interpolation
    patch_pos_embed = patch_pos_embed.reshape(1, D, int(math.sqrt(N // D)), int(math.sqrt(N // D)), dim).permute(0, 4, 1, 2, 3)
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        # Mark: the actual shape of MRI may be [C, W, H, D]
        # But it is OK as W == H == D
        B, C, D, H, W = x.shape
        # [B, C, D, H, W] -> [B, C, Dh, Hh, Wh]
        x = self.proj(x)
        dhw_shape = x.shape[2:]
        # [B, C, Dh, Hh, Wh] -> [B, L, C]
...
class GroupViT(nn.Module):
    ...
    def __init__(self,
                 ...
                 depths=[6, 3, 3],
                 num_heads=[6, 6, 6],
                 num_group_tokens=[8, 2, 0],
                 num_output_groups=[8, 2],
        ...
        if pos_embed_type == 'simple':
            ...
            self.pos_embed = self.build_3d_sincos_position_embedding()
        else:
            raise ValueError
    ...
    def load_state_dict(self, state_dict: 'OrderedDict[str, torch.Tensor]', strict: bool = True):
        if self.pos_embed_type == 'simple' and 'pos_embed' in state_dict:
            ...
            # Ensure that pos_embed is 3D
            if load_pos_embed.shape != pos_embed.shape:
                D_new = self.patch_embed.patches_resolution[0]
                H_new = self.patch_embed.patches_resolution[1]
                W_new = self.patch_embed.patches_resolution[2]
                
                D_ori = load_pos_embed.shape[1]  # Original depth
                H_ori = int(load_pos_embed.shape[2]**0.5)  # Original height
                W_ori = H_ori  # Assuming square input for original
                
                # Reshape original load_pos_embed for 3D interpolation
                load_pos_embed = rearrange(load_pos_embed, 'b (d h w) c -> b c d h w', d=D_ori, h=H_ori, w=W_ori)
                ...
                load_pos_embed = rearrange(load_pos_embed, 'b c d h w -> b (d h w) c')
    ...
    def build_3d_sincos_position_embedding(self, temperature=10000.):
        d, h, w = self.patch_embed.patches_resolution  # Assuming patches_resolution is a tuple (depth, height, width)
    ...
    def get_pos_embed(self, D, H, W):
        if self.training:
            return self.pos_embed
        pos_embed = self.pos_embed
        pos_embed = interpolate_3d_pos_encoding(pos_embed, D, H, W)
        return pos_embed
    ...
    def forward_features(self, x, *, return_attn=False):
        ...
        x = x + self.get_pos_embed(*dhw_shape)
        x = self.pos_drop(x)
